...
class Mirai(metaclass=Singleton):
    ...
    def handle_group_invation(self, eventId, fromId, groupId, operation):
        """处理群邀请信息
        :param eventId: 邀请事件id
        :param fromId: 邀请人Id
        :param groupId: 邀请进入的群的Id
        ...
        if self.adapter == 'http':
            response = self.__session.post(url=f'{self.base_url}/resp/botInvitedJoinGroupRequestEvent', json=content).json()
        else:
            assert self.adapter == 'ws'
            response = self.__ws_send(command='resp/botInvitedJoinGroupRequestEvent', content=content)
        return response
